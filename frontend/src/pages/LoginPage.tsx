...
export function LoginPage() {
  ...
  const handleBotAuth = useCallback(async () => {
    ...
    try {
      ...
      // Build tg:// protocol link — works reliably on iOS PWA standalone
      // where https://t.me/ universal links often fail
      if (result.bot_username && result.code) {
        setBotDeepLink(`tg://resolve?domain=${result.bot_username}&start=auth_${result.code}`);
      } else if (result.deep_link && result.deep_link.startsWith("https://t.me/")) {
        // Fallback to https://t.me/ if bot_username not available
        setBotDeepLink(result.deep_link);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const poll = setInterval(() => {
      void auth.checkBotAuth(botAuthCode).then((bundle) => {
        ...
        errorCount = 0; // Reset on any successful poll
      }).catch(() => {
        errorCount++;
        if (errorCount >= 3) {
          clearInterval(poll);
          ...
          setBotAuthCode(null);
          setBotDeepLink(null);
          setBotAuthError("Ошибка авторизации. Попробуйте снова.");
        }
      });
    }, 2000);
      ...
      <div className="auth-card">
        ...
        <div style={{ display: "flex", justifyContent: "center" }}>
          ...
          >
            ...
            {botAuthBusy
              ? (botAuthCode ? "Ожидаем подтверждение…" : "Подключаемся…")
        ...
        {botAuthCode ? (
          <div style={{ display: "grid", gap: "10px", marginTop: 8 }}>
            {botDeepLink && (
              <a href={botDeepLink} className="auth-telegram-open">
