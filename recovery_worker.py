    ...
    async def _run_recovery(self, task: dict) -> dict:
        ...
        report = {
            ...
            "session_auth_audit": await audit_sessions(
                ...
                authorized_stage=task.get("authorized_stage") if task.get("authorized_stage") is not None else "active_commenting",
                authorized_status=task.get("authorized_status") if task.get("authorized_status") is not None else "active",
                authorized_health=task.get("authorized_health") if task.get("authorized_health") is not None else "alive",
